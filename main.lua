...
local function urlscript(scripturl)
	if shared.VapeDeveloper then
		if not betterisfile("vape/"..scripturl) then
			error("File not found : vape/"..scripturl)
		end
		...
		local res = game:HttpGet("https://raw.githubusercontent.com/NTDCore/nobolineback/main/"..scripturl, true)
		assert(res ~= "404: Not Found", "File not found")
		return res
	end
end
...
local checkpublicreponumn = 0
local checkpublicrepon
checkpublicrepon = function(id)
	local suc, req = pcall(function() return requestfunc({
		Url = "https://raw.githubusercontent.com/NTDCore/nobolineback/main/CustomModules/"..id..".lua",
		Method = "GET"
	}) end)
	if not suc then
		checkpublicreponumn = checkpublicreponumn + 1
		spawn(function()
...
if shared.VapeIndependent then
	...
	if betterisfile("vape/CustomModules/"..game.PlaceId..".lua") then
		...
		local publicrepo = checkpublicrepon(game.PlaceId)
		if publicrepon then
			loadstring(publicrepon)()
		end
	end
	if shared.VapePrivate then
